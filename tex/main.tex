...
We evaluate mathematical components through controlled simulations and describe three exploratory real-data applications (\Cref{sec:experiments}). The synthetic belief network is a constructed separation check. Earlier cryptocurrency, EEG, and Reddit outputs are not reproduced by the present checkout and cannot presently validate transfer, cooperation, community type, virtue, or moral alignment.
...
We test mathematical components in constructed simulations and report historic, exploratory outputs from three real-data applications. Experiment~1 checks whether deliberately parameterized synthetic regimes can be separated; the cryptocurrency extension is unreproduced. Experiment~2 applies Forman--Ricci curvature to an EEG task dataset, but the historic analysis confounds condition with block order and treats autocorrelated windows as independent, so it must be redesigned. Experiment~3 applies persistent homology to Sentence-BERT embeddings \citep{reimers2019sentence} from the Politosphere corpus \citep{hofmann2022politosphere}; it describes embedding-cloud geometry rather than an identified belief attractor. These pipelines share mathematical components, but their domain-specific measurement and inferential assumptions are not interchangeable. \Cref{tab:hypothesis_mapping} records the status of each target.
...
H3: Curvature predicts condition & 2 & Historic exploratory output; current inference invalid, redesign required \\
...
The historic H3 pipeline analyzed EEG hyperscanning data from tasks instructed as cooperative or competitive \citep{czeszumski2022eeg}. It concatenated all cooperation recordings before all competition recordings, computed phase-locking value on inter-brain channel pairs in non-overlapping windows, and correlated smoothed Forman--Ricci curvature with the binary task label. This design tests neither cooperation nor a directionally specified neural mechanism: condition is confounded with a single order/time transition, and the window-wise test does not respect the dyad as the independent unit.

The archived manuscript reported per-dyad correlations ranging from $-0.56$ to $0.74$ and summarized their absolute values. Those statistics have no machine-readable ledger or data in this checkout and were not reproduced. Mixed signs, post-hoc absolute-value aggregation, autocorrelation, and the order confound prevent the old $p$ values from supporting a general curvature effect. A valid rerun must retain task/block order, use dyad-level hierarchical or randomization inference, freeze smoothing and threshold sensitivity, and validate behavioral joint outcomes before using the word cooperation.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{fig_exp2_curvature.pdf}
  \caption{Historic exploratory EEG output for the dyad with the largest archived correlation. Because all cooperation blocks precede all competition blocks, the visible transition is confounded with time/order and is not evidence that curvature identifies cooperation. The data and result ledger are absent from the current checkout.}
...
\begin{figure}[!htbp]
  ...
  \caption{Historic exploratory per-dyad curvature--condition correlations. Stars and absolute-value summaries reproduce the earlier figure annotation, not a valid current inference: direction was heterogeneous, windows were autocorrelated, condition was order-confounded, and the analysis was not reproduced.}
...
The historic Experiment~3 applied persistent homology and distributional distances to text embeddings from the Reddit Politosphere corpus \citep{hofmann2022politosphere}. It selected 8 subreddits and assigned three authored community-type labels:
\begin{itemize}[nosep]
...
The historic persistence outputs varied across the authored labels (\Cref{fig:exp3_persistence}), but they were not reproduced in this audit. Even if reproduced, topology of a Sentence-BERT point cloud is geometry of sampled model embeddings, not an identified attractor of community beliefs. Counts and persistence values therefore cannot by themselves establish ideological cohesion, echo chambers, polarization, or moral alignment. Those interpretations require independent blinded outcomes, a shared representation transform, sampling sensitivity, and adequate ordered trajectories.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{fig_exp3_persistence.pdf}
  \caption{Historic exploratory persistence summaries of Reddit embedding clouds, grouped by authored community labels. These unreproduced differences describe sampled embedding geometry and do not establish echo chambers, polarization, or belief attractors.}
  \label{fig:exp3_persistence}
\end{figure}

The archived metric comparison reported a KL label-discrimination ratio of $1.70$, negligible centroid discrimination, and degenerate RDS values. These figures are exploratory and unreproduced. The RDS result is non-estimability rather than evidence: one or two monthly centroids cannot identify the Koopman dynamics. A redesigned study needs a frozen shared representation, sufficient ordered time bins, time-respecting validation, and outcomes independent of subreddit names.
...
\begin{table}[h]
\centering
\caption{Historic exploratory per-dyad EEG correlations; current inferential design invalid and unreproduced.}
