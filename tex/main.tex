...
We validate the framework through three experiments (\Cref{sec:experiments}): a synthetic belief network demonstrating that RDS distances capture structural misalignment invisible to KL divergence; a simulated EEG hyperscanning dyad showing that curvature dynamics predict cooperation and rupture; and simulated social media communities revealing echo chambers as topologically distinct belief basins with measurably high inter-basin distances.
...
We validate the framework through three synthetic experiments targeting five integrated hypotheses. All experiments use simulated data with known generative parameters, providing controlled ground-truth validation. Extension to real-world data (EEG hyperscanning, social media corpora) is an immediate next step requiring only data preprocessing, as the analysis pipelines are data-agnostic. \Cref{tab:hypothesis_mapping} summarizes the mapping.
...
For hypothesis H2, we compute group-level belief trajectories (centroids over time) and estimate both the KL divergence between fitted Gaussian marginals and the RDS distance via Koopman operators estimated by Dynamic Mode Decomposition. We compare the rank orderings produced by the two metrics: if RDS distance captures structural information beyond marginals, the two metrics should produce different rank orderings of group pairs, reflecting the dynamical structure that KL compresses away.

For hypothesis H4, at $t = 5000$ we reduce Group R's precision by a factor of four ($\pi_i \to \pi_i / 4$), track trajectories before and after perturbation, and verify that the direction of belief change correlates with the gradient of curvature on the belief landscape. For hypothesis H5, we estimate geometric empowerment for each group by measuring the effective dimensionality of the reachable belief set under observation perturbations (spectral entropy of the one-step reachable covariance, scaled by total variance). Rigid agents with high precision should show lower empowerment (fewer distinct reachable states), while flexible agents should show higher empowerment, consistent with the interpretation that empowered agents inhabit shallower basins with more available trajectories, as shown in \Cref{fig:exp1_combined}(d).

\subsection{Experiment 2: EEG Hyperscanning Dyad}
\label{sec:exp2}

To test whether curvature dynamics predict cooperative synchronization (H3), we simulate dual-participant neural recordings during a joint task using coupled Kuramoto oscillators with time-varying coupling strength. Each participant is modelled as a population of 16 oscillators with natural frequencies drawn uniformly from the alpha band (8--12~Hz); inter-brain coupling oscillates between synchronized and independent phases at 0.05~Hz, simulating the natural ebb and flow of cooperative engagement.
...
To test whether persistent homology distinguishes structurally different online communities (H1) and whether topological metrics capture inter-community differences (H2), we simulate 10 communities across three structural types: echo chambers (4 communities with tightly clustered opinions around a single center, low variance, high within-community similarity), diverse communities (3 communities with diffuse opinions, high variance, multiple sub-clusters), and polarized communities (3 communities with bimodal opinion distributions, reflecting internal factional splits).

Each community generates 5{,}000 opinion vectors in $\R^{50}$ with type-specific generative parameters. We reduce dimensionality via PCA, compute persistent homology on each community's point cloud, and estimate pairwise distances using three metrics: the RDS distance (via Koopman operators estimated from 156 weekly centroids), KL divergence (between fitted Gaussian distributions on PCA-reduced embeddings), and embedding distance (centroid-to-centroid Euclidean).

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{fig_exp3_distances.pdf}
  \caption{Pairwise distance matrices between 10 synthetic communities, computed using three metrics: RDS distance (left), KL divergence (center), and embedding distance (right). All matrices are normalized to $[0,1]$ for comparison. KL divergence shows the clearest block structure discriminating community types, while RDS distance captures subtler dynamical differences between communities of the same type. Echo chambers cluster tightly in all metrics.}
...
The most immediate extension is validation on real-world data: EEG hyperscanning datasets (e.g., from OpenNeuro) for Experiment~2 and Reddit corpora (via the Pushshift archive) for Experiment~3, using the same analysis pipelines with Sentence-BERT embeddings and Forman--Ricci curvature on empirical interaction graphs. Further extensions include a quantum-enhanced pipeline using quantum TDA and quantum reservoir computing for more efficient attractor reconstruction \citep{lloyd2016quantum}, integration with mechanistic interpretability to identify which internal representations correspond to attractor features, and real-time co-steering applications in human--AI teams using the curvature diagnostic as a live alignment monitor.
...
Our experimental validation demonstrates that topological structure predicts alignment dynamics across synthetic, neural, and social data. Persistent homology reliably reconstructs attractor landscapes. In simulated hyperscanning data, curvature dynamics on interaction graphs predict cooperative synchronization with $r = 0.81$, with curvature changes preceding coupling changes by approximately 10 seconds. Topological metrics distinguish community types through their persistence signatures, though accurate Koopman-based comparison requires sufficient temporal data.
...
The synthetic dual-EEG simulation uses coupled Kuramoto oscillators. Each participant is modelled as 16 oscillators with natural frequencies drawn uniformly from the alpha band (8--12~Hz). The coupling strength between participants oscillates sinusoidally at 0.05~Hz between 0 (independent) and 0.6 (synchronized). Phase-locking values are computed in non-overlapping 2-second windows; Forman--Ricci curvature is computed on the resulting interaction graph at each window. A 3-block moving average smoothing kernel is applied to the curvature time series before correlation analysis.

\subsection{Experiment 3: Community Simulation}

Ten synthetic communities are generated in $\R^{50}$. Echo chamber communities (4) have tightly clustered opinions around structured centers at $[5, 5, 0, 0, \ldots]$ with low variance ($\sigma = 0.3$). Diverse communities (3) have diffuse opinions centered at $[-5, -3, 4, 0, \ldots]$ with high variance ($\sigma = 0.6$) and 3--5 sub-clusters offset by $\sigma = 1.5$. Polarized communities (3) have bimodal distributions centered at $[0, -5, -5, 0, \ldots]$ with poles separated by 6 units and per-pole noise $\sigma = 0.5$. Each community generates 5{,}000 opinion vectors. Persistent homology uses Vietoris--Rips filtration on 500 subsampled points with a persistence threshold of 0.5 for $H_0$ component counting. RDS estimation uses Dynamic Mode Decomposition on 156 weekly snapshots of community embedding centroids.
