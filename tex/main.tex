...
We validate the framework through three controlled synthetic experiments (\Cref{sec:experiments}): a belief network of precision-weighted agents demonstrating that RDS distances capture structural misalignment invisible to KL divergence; coupled Kuramoto oscillators showing that Forman--Ricci curvature dynamics predict cooperative synchronization; and simulated online communities revealing echo chambers as topologically distinct belief basins with measurably different persistence signatures.
...
\begin{equation}
  ...
  \label{eq:efe}
\end{equation}
where $C$ encodes preferred observations and $p(s_\tau \mid o_\tau)$ is the generative model's posterior. The terms $\ln q(s_\tau \mid \pi) - \ln p(s_\tau \mid o_\tau)$ capture the expected divergence between the agent's approximate posterior and the true posterior (combining ambiguity and epistemic drive), while $-\ln p(o_\tau \mid C)$ captures risk: the expected surprisal of predicted observations relative to preferences. The EFE equivalently decomposes into risk and information gain \citep{parr2022active}. Policy selection follows a softmax $p(\pi) = \sigma\bigl(-\gamma \, \EFE(\pi)\bigr)$ with precision parameter $\gamma$ controlling the exploitation--exploration trade-off.
...
We validate the framework through three controlled synthetic experiments targeting four integrated hypotheses. Each experiment provides ground-truth validation with known generative parameters, ensuring that observed topological signatures reflect genuine structural differences rather than data artifacts. Experiment~1 simulates a belief network of precision-weighted agents with distinct attractor geometries. Experiment~2 uses coupled Kuramoto oscillators to model inter-brain synchronization during cooperation and rupture. Experiment~3 generates synthetic online communities with echo chamber, diverse, and polarized structures. The analysis pipelines are data-agnostic: the same TDA, Koopman, and curvature computations apply equally to synthetic and real data (e.g., cryptocurrency markets, EEG/fNIRS hyperscanning, Reddit corpora) without modification. \Cref{tab:hypothesis_mapping} summarizes the mapping.
...
\midrule
H1: Attractors are reconstructable & 1, 2, 3 & TDA persistence ratio $= 2.43\times$ (Exp~1); persistence hierarchy (Exp~3) \\
H2: RDS distance $\neq$ KL for structural alignment & 1, 3 & Rank reversal between metrics (Exp~1); complementary discrimination (Exp~3) \\
H3: Curvature predicts cooperation & 2 & Curvature--coupling $r = 0.81$, $p < 0.001$ \\
H4: Interventions bend geodesics & 1 & Direction change $= 0.55$ after precision perturbation \\
\bottomrule
\end{tabular}
\end{table}
...
For hypothesis H1, we run 10{,}000 time steps, compute group centroid trajectories, and apply Takens embedding ($\tau = 5$, $d_e = 8$) followed by persistent homology via Vietoris--Rips filtration. The primary metric is the persistence ratio between Flexible and Rigid group centroids: a ratio significantly greater than 1 confirms that the topological pipeline correctly identifies Flexible attractors as more complex (higher-dimensional, more diffuse recurrence) than Rigid attractors. We obtain a persistence ratio of $2.43\times$ (Flexible mean persistence $= 0.030$, Rigid $= 0.012$, Mixed $= 0.018$), confirming that TDA reliably distinguishes attractor geometries arising from different precision regimes.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{fig_exp1_combined.pdf}
  \caption{Experiment~1 results. (a)~Belief volatility over time for three agent groups. The precision perturbation at $t = 5000$ increases Rigid group volatility (H4), producing a direction change of $0.55$. (b)~$H_0$ persistence barcodes from Takens-embedded group centroid trajectories; Flexible attractors show $2.43\times$ higher mean persistence than Rigid, confirming that TDA captures the attractor geometry difference (H1). (c)~RDS vs.\ KL distance between groups. The rank orderings differ: KL ranks R--F $<$ F--M $<$ R--M, while RDS ranks R--F $<$ R--M $<$ F--M, demonstrating that Koopman-based comparison captures dynamical structure invisible to distributional metrics (H2).}
...
For hypothesis H4, at $t = 5000$ we reduce Group R's precision by a factor of four ($\pi_i \to \pi_i / 4$), track trajectories before and after perturbation, and measure the cosine distance between pre- and post-perturbation belief velocity vectors. The direction change magnitude is $0.55$ (cosine similarity drops from $1.0$ to $0.45$), confirming that precision perturbation bends belief geodesics substantially. This demonstrates that the thermodynamic parameter $\gamma$ (inverse temperature / precision) serves as an effective control knob for steering attractor geometry, as predicted by the Phase~3 analysis.

\paragraph{Extensibility to real data.} The same pipeline applies directly to empirical time series without modification. For instance, daily OHLCV cryptocurrency data can be embedded via feature vectors $[r_t, \hat{\sigma}_{5d}, v_t / \bar{v}, \Delta_{\mathrm{HL}}]$, classified by volatility regime, and subjected to the same Takens embedding and persistence analysis. Similarly, EEG/fNIRS hyperscanning or social media embedding trajectories can be processed through identical TDA and Koopman pipelines. We leave systematic real-data validation to future work.

\subsection{Experiment 2: Hyperscanning Dyad Simulation}
\label{sec:exp2}

To test whether curvature dynamics predict cooperative synchronization (H3), we simulate dual-participant neural recordings using coupled Kuramoto oscillators with time-varying coupling strength \citep{hakim2023}. Each participant is modelled as a population of 16 oscillators with natural frequencies drawn uniformly from the alpha band (8--12~Hz); inter-brain coupling oscillates between synchronized ($\kappa = 0.6$) and independent ($\kappa = 0$) phases at 0.05~Hz, producing 100 seconds of data at 500~Hz. We compute the phase-locking value (PLV) between all inter-brain oscillator pairs ($8 \times 8 = 64$ channel pairs) in non-overlapping 2-second windows, yielding a time-varying interaction graph with 50 snapshots. Forman--Ricci curvature (\Cref{eq:forman_ricci}) is computed on this graph at each window, producing a curvature time series. A 3-block (${\sim}6$~s) smoothing kernel removes high-frequency noise while preserving the coupling-related signal.

As shown in \Cref{fig:exp2_curvature}, the curvature--coupling correlation achieves $r = 0.81$ ($p < 0.001$), with curvature changes preceding coupling by approximately 10 seconds (lag $= -5$ windows). This temporal precedence is consistent with the Phase~4 prediction that curvature on the interaction graph reflects convergent or divergent belief flows: positive curvature (convergence) builds before behavioral synchronization manifests, and negative curvature (divergence) anticipates rupture. The Forman--Ricci curvature computation is modality-agnostic: it operates on weighted interaction graphs regardless of whether edge weights derive from PLV, wavelet coherence, or other synchrony measures, making the pipeline directly applicable to real EEG and fNIRS hyperscanning data \citep{czeszumski2022eeg, nguyen2022parentchild, berretz2025socialtouch}.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{fig_exp2_curvature.pdf}
  \caption{Experiment~2 results: Forman--Ricci curvature dynamics on the simulated inter-brain interaction graph. Top: raw and smoothed curvature time series (50 non-overlapping 2\,s windows). Middle: ground-truth coupling strength (sinusoidal oscillation at 0.05~Hz). Bottom: z-score normalized overlay showing the curvature--coupling relationship ($r = 0.81$, $p < 0.001$). Curvature leads coupling by ${\sim}10$~s (lag $= -5$ windows), consistent with the prediction that interaction graph geometry anticipates behavioral synchronization.}
  \label{fig:exp2_curvature}
\end{figure}

\subsection{Experiment 3: Simulated Online Community Belief Basins}
\label{sec:exp3}

To test whether persistent homology distinguishes structurally different online communities (H1) and whether topological metrics capture inter-community differences (H2), we generate 10 synthetic communities in $\R^{50}$ with known structural parameters spanning three types:
\begin{itemize}[nosep]
  \item \textbf{Echo chambers} (4): Tight Gaussian clusters ($\sigma = 0.3$) with slow drift ($0.005$ per week) and high temporal autocorrelation, modeling communities with strong in-group consensus and ideological homogeneity.
  \item \textbf{Diverse} (3): Broad distributions ($\sigma = 0.6$) with 3--5 sub-clusters and fast drift ($0.08$ per week), modeling communities with explicit viewpoint diversity and active debate.
  \item \textbf{Polarized} (3): Bimodal distributions with poles separated by 6 units in embedding space ($\sigma = 0.5$ per pole) and moderate drift ($0.02$ per week), modeling communities with internal factional dynamics.
\end{itemize}

Each community contains 5{,}000 synthetic posts distributed across 156 weeks, embedded in $\R^{50}$ with type-specific centers separated along interpretable axes (political orientation, epistemic openness). Community types are distinguished not by location in embedding space but by the \emph{geometry} of their attractor: cluster tightness, number of sub-modes, and temporal drift rate. We compute persistent homology on each community's embedding point cloud using Vietoris--Rips filtration with adaptive filtration radius (95th percentile of pairwise distances), and estimate pairwise distances using three metrics: RDS distance (via Koopman operators estimated from weekly centroids), KL divergence (between fitted Gaussian distributions), and embedding distance (centroid-to-centroid Euclidean). The same pipeline is directly applicable to real social media data such as the Reddit Politosphere corpus \citep{hofmann2022politosphere} using Sentence-BERT embeddings \citep{reimers2019sentence}.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{fig_exp3_distances.pdf}
  \caption{Pairwise distance matrices between 10 synthetic communities, computed using three metrics: RDS distance (left), KL divergence (center), and embedding distance (right). All matrices are normalized to $[0,1]$ for comparison. Same-type communities cluster together across all metrics, with KL divergence providing the strongest discrimination (same-type/different-type ratio $= 3.88$), followed by embedding distance ($1.55$) and RDS distance ($1.17$). The weaker RDS discrimination reflects the short temporal trajectories available in the community setting, where distributional metrics have more statistical power than dynamical ones.}
  \label{fig:exp3_distances}
\end{figure}

The persistence analysis reveals topological signatures that cleanly distinguish community types (\Cref{fig:exp3_persistence}). Echo chamber communities show low mean $H_0$ persistence ($4.54$), reflecting tightly clustered opinion distributions around a single ideological center where connected components merge at small filtration radii. Polarized communities show intermediate persistence ($18.00$), consistent with bimodal opinion landscapes whose two poles merge at moderate radii. Diverse communities show the highest persistence ($71.76$), consistent with multi-modal opinion landscapes with several persistent subclusters that survive to large filtration radii. The ordering (Echo $<$ Polarized $<$ Diverse) spans more than an order of magnitude and is consistent with the known structural parameters. The adaptive filtration radius (95th percentile of pairwise distances per community) prevents artificial ceiling effects that would mask these differences.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{fig_exp3_persistence.pdf}
  \caption{$H_0$ persistence barcodes for representative communities of each type (top 40 features by persistence, computed via Vietoris--Rips filtration on 500 subsampled points with adaptive filtration radius). Echo chambers (mean persistence $= 4.54$) show short bars reflecting tight clustering. Polarized communities ($18.00$) show intermediate bars reflecting bimodal structure. Diverse communities ($71.76$) show long bars consistent with broad, multi-modal opinion landscapes. Dashed lines indicate mean persistence per community type.}
  \label{fig:exp3_persistence}
\end{figure}

The metric comparison yields a nuanced finding. KL divergence achieves the strongest discrimination ratio between same-type and different-type community pairs ($3.88$), followed by embedding distance ($1.55$) and RDS distance ($1.17$). This ordering reflects the short temporal trajectories available (156 weekly centroids per community): distributional metrics have more statistical power than dynamical ones when time series are limited. However, the three metrics capture complementary information. KL divergence measures static distributional differences (cluster spread and location). Embedding distance captures geometric separation in the ambient space. RDS distance captures dynamical differences in how community opinions evolve over time, including drift rate and temporal autocorrelation. In settings with longer observation periods, we expect the RDS metric to provide stronger discrimination, as demonstrated in Experiment~1 where the rank reversal between RDS and KL confirms that dynamical structure contains information invisible to distributional metrics.
...
Several challenges remain. Persistent homology scales as $O(n^3)$ in the number of simplices, requiring approximations such as witness complexes or subsampling for large point clouds. Koopman operator estimation requires long, stationary time series; nonstationarity from regime changes demands windowed estimation with appropriate change-point detection. As demonstrated in Experiment~3, RDS distance underperforms distributional metrics when temporal trajectories are short, suggesting that the framework is most powerful in settings with extended observation periods. The Mori--Zwanzig projection requires choosing a timescale separation that may not be unique or obvious in real systems. The four normative criteria of Phase~4.5 are proposed as sufficient conditions for beneficial attractors, but their necessity and the trade-offs between them require further philosophical and empirical work.

The synthetic experiments demonstrate that the framework's analysis pipelines are sound: TDA reliably distinguishes attractor geometries arising from different precision regimes (persistence ratio $2.43\times$); Forman--Ricci curvature on interaction graphs predicts cooperative synchronization with high fidelity ($r = 0.81$, $p < 0.001$); and persistence signatures cleanly separate community types spanning an order of magnitude (Echo $= 4.54$, Polarized $= 18.00$, Diverse $= 71.76$). Crucially, the pipelines are data-agnostic: the same computations apply to any setting that produces belief trajectories, interaction graphs, or embedding point clouds. Immediate next steps include validation on real-world data, including cryptocurrency market time series, EEG/fNIRS hyperscanning dyads from published open-access datasets \citep{czeszumski2022eeg, nguyen2022parentchild, berretz2025socialtouch}, and Reddit political communities from the Politosphere corpus \citep{hofmann2022politosphere}.
...
Our experimental validation demonstrates that topological structure predicts alignment dynamics across synthetic belief networks, neural oscillator models, and simulated online communities. Persistent homology reliably reconstructs attractor landscapes, distinguishing flexible from rigid belief dynamics with a persistence ratio of $2.43\times$. In simulated hyperscanning data, curvature dynamics on interaction graphs predict cooperative synchronization ($r = 0.81$, $p < 0.001$) with curvature leading coupling by ${\sim}10$~s. Topological metrics applied to simulated online communities distinguish echo chambers, polarized, and diverse community structures through persistence signatures spanning an order of magnitude ($4.54$ to $71.76$). The analysis pipelines are data-agnostic and designed for direct application to real-world datasets including cryptocurrency markets, EEG/fNIRS hyperscanning, and social media corpora.
...
\subsection{Experiment 2: Processing Pipeline}

The synthetic dual-EEG simulation uses coupled Kuramoto oscillators. Each participant is modelled as 16 oscillators with natural frequencies drawn uniformly from the alpha band (8--12~Hz). The coupling strength oscillates sinusoidally at 0.05~Hz between 0 (independent) and 0.6 (synchronized), producing 100 seconds of data at 500~Hz (50{,}000 samples per channel). Phase-locking values are computed between 8 inter-brain channel pairs in non-overlapping 2-second windows (50 blocks total). The PLV threshold for edge inclusion in the interaction graph is 0.3. Curvature is smoothed with a 3-block moving average kernel (${\sim}6$~s). Time-lagged cross-correlation uses a maximum lag of $\pm 5$ windows ($\pm 10$~s). The pipeline is modality-agnostic: for fNIRS data, wavelet coherence (Morlet wavelets, 0.01--0.1~Hz) would replace PLV, with 30~s windows to match the hemodynamic signal's lower frequency content.

\subsection{Experiment 3: Synthetic Community Pipeline}

\paragraph{Data generation.} We generate 10 synthetic communities in $\R^{50}$: 4 echo chambers ($\sigma = 0.3$, drift rate $0.005$/week), 3 diverse ($\sigma = 0.6$, 3--5 sub-clusters, drift rate $0.08$/week), and 3 polarized (bimodal, poles separated by 6 units, drift rate $0.02$/week). Each community contains 5{,}000 posts distributed across 156 weeks. Community type centers are separated along interpretable axes (political orientation, epistemic openness) with inter-type separation $\geq 10$ and intra-type jitter $\leq 1.5$.

\paragraph{Analysis.} Persistent homology uses Vietoris--Rips filtration on 500 subsampled points with adaptive filtration radius (95th percentile of pairwise distances from a 200-point subsample) and a persistence threshold of 0.5 for $H_0$ component counting. For pairwise distance computation, KL divergence uses PCA projection to 20 dimensions for numerical stability. RDS distance uses Koopman operators estimated from weekly centroids via DMD with rank 5. The same pipeline is designed for direct application to real Reddit data embedded via Sentence-BERT (all-mpnet-base-v2) \citep{reimers2019sentence}.


\section{Supplementary Results}
\label{app:supplementary}

\paragraph{Experiment~1: Numerical results summary.}
\Cref{tab:exp1_results} summarizes the key numerical results from Experiment~1.

\begin{table}[h]
\centering
\caption{Experiment~1 numerical results.}
\label{tab:exp1_results}
\begin{tabular}{@{}lll@{}}
\toprule
Hypothesis & Metric & Value \\
\midrule
H1 & Persistence ratio (F/R) & $2.43\times$ \\
    & Rigid mean persistence & $0.012$ \\
    & Flexible mean persistence & $0.030$ \\
    & Mixed mean persistence & $0.018$ \\
\midrule
H2 & RDS: R--F / R--M / F--M & $0.022$ / $0.030$ / $0.043$ \\
    & KL: R--F / R--M / F--M & $15.3$ / $37.7$ / $23.2$ \\
    & Rank reversal & R--M $<$ R--F (RDS) vs.\ R--F $<$ F--M (KL) \\
\midrule
H4 & Direction change magnitude & $0.55$ \\
    & Pre/post cosine similarity & $0.45$ \\
\bottomrule
\end{tabular}
\end{table}
