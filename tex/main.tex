...
\begin{figure}[t]
  ...
  \caption{Experiment~1 results. (a)~Belief volatility (rolling mean of step-to-step belief change $\|\Delta\phi\|$) over time. Rigid agents show low volatility consistent with a deep attractor basin; Flexible agents show high volatility consistent with a shallow basin; Mixed agents are intermediate. The precision perturbation at $t = 5000$ (reducing $\pi_i$ by a factor of four for Group~R) visibly increases their volatility, demonstrating that interventions on precision bend belief geodesics (H4). (b)~$H_0$ persistence barcodes: Rigid agents show long-persisting features reflecting structured, slowly converging trajectories with coherent attractor dynamics. Flexible agents show shorter bars reflecting rapid, noise-dominated dynamics consistent with a shallow basin. (c)~RDS misalignment distance and KL divergence between groups (both normalized). The two metrics give different rank orderings, reflecting the structural information captured by the Koopman-based comparison. (d)~Geometric empowerment by group, confirming that rigid agents inhabit low-empowerment regions while flexible agents retain capacity for belief revision.}
