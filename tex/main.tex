...
\begin{proof}[Proof sketch]
For exponential families, the map $\phi \mapsto q(\cdot; \phi)$ is a diffeomorphism from natural parameters to mean parameters. The Fisher--Rao metric in natural coordinates equals the Hessian of the log-partition function, so $g_{ij} = \partial^2 A / \partial \phi_i \partial \phi_j$. Since $\mu = \nabla A(\phi)$, we have $\dot{\mu} = \nabla^2 A(\phi) \, \dot{\phi} = -\nabla_\phi \VFE$: the natural gradient flow in $\phi$-coordinates corresponds to Euclidean gradient flow in mean coordinates. The Bregman divergence $D_A(\phi \| \phi') = A(\phi) - A(\phi') - \nabla A(\phi')^\top (\phi - \phi')$ equals the KL divergence for exponential families. By the Otto calculus \citep{otto2001geometry}, the Bregman divergence on the finite-dimensional exponential family submanifold agrees with the Wasserstein distance up to $O(\eta^2)$ curvature corrections for small steps. Matching the JKO implicit Euler scheme term by term yields the result. The correction vanishes for Gaussian families where $\nabla^3 A = 0$.
\end{proof}
